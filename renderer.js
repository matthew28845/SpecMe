...
window.specs.battery().then(info => {
  ...
  batterytable.querySelector('.battmaxcap').innerText = `${info.maxCapacity} ${info.capacityUnit}`
  batterytable.querySelector('.batthealth').innerText = `${Math.floor((info.maxCapacity/info.designedCapacity)*100)}%`
